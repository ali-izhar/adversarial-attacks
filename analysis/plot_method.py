...
def plot_success_rates(json_data, methods, norm_type, output_dir="plots"):
    ...
    available_methods = [
        method for method, norm in json_data.keys() if norm == norm_type
    ]
    ...
    plt.tight_layout()
    plt.savefig(
        f"{output_dir}/success_rates_{norm_type}.pdf", bbox_inches="tight", dpi=300
    )
    plt.savefig(
        f"{output_dir}/success_rates_{norm_type}.png", bbox_inches="tight", dpi=300
    )
    plt.close()
...
def plot_perturbation_metrics(json_data, methods, norm_type, output_dir="plots"):
    ...
    plt.savefig(
        f"{output_dir}/perturbation_metrics_{norm_type}.pdf",
        bbox_inches="tight",
        dpi=300,
    )
    plt.savefig(
        f"{output_dir}/perturbation_metrics_{norm_type}.png",
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def plot_computational_metrics(json_data, methods, norm_type, output_dir="plots"):
    ...
    plt.subplots_adjust(bottom=0.2)
    plt.savefig(
        f"{output_dir}/computational_metrics_{norm_type}.pdf",
        bbox_inches="tight",
        dpi=300,
    )
    plt.savefig(
        f"{output_dir}/computational_metrics_{norm_type}.png",
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def plot_method_comparison(json_data, methods, norm_type, output_dir="plots"):
    ...
    max_linf = max(
        [
            method_metrics[method]["linf_norm"]
            for method in methods
    ...
    for method in methods:
        if method_metrics[method]["l2_norm"] > 0:
            method_metrics[method]["l2_norm_norm"] = (
                method_metrics[method]["l2_norm"] / max_l2
            )
        else:
            method_metrics[method]["l2_norm_norm"] = 0

        if method_metrics[method]["linf_norm"] > 0:
            method_metrics[method]["linf_norm_norm"] = (
                method_metrics[method]["linf_norm"] / max_linf
            )
        else:
    ...
    plt.savefig(
        f"{output_dir}/method_comparison_{norm_type}.pdf", bbox_inches="tight", dpi=300
    )
    plt.savefig(
        f"{output_dir}/method_comparison_{norm_type}.png", bbox_inches="tight", dpi=300
    )
    plt.close()
...
def plot_perturbation_visualizations(csv_data, methods, norm_type, output_dir="plots"):
    ...
    for metric, title in zip(metrics, titles):
        ...
        if grouped_data:
            ...
            plt.tight_layout()
            plt.savefig(
                f"{output_dir}/{metric}_distribution_{norm_type}.pdf",
                bbox_inches="tight",
                dpi=300,
            )
            plt.savefig(
                f"{output_dir}/{metric}_distribution_{norm_type}.png",
                bbox_inches="tight",
                dpi=300,
            )
            plt.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot adversarial attack evaluation results"
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    parser.add_argument(
        "--methods",
        nargs="+",
        default=["FGSM", "FFGSM", "DeepFool", "CW"],
    ...
    print(
        f"Loading results for methods: {args.methods}, norm: {args.norm}, targeted: {args.targeted}"
    )
    csv_data, json_data = load_results(
        methods=args.methods, norm_types=[args.norm], targeted=args.targeted
    )
