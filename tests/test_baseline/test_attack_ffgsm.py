"""Tests for the Fast FGSM (FFGSM) adversarial attack implementation."""
...
class TestFFGSMAttack:
    """Tests for the FFGSM adversarial attack."""
    ...
    def test_least_likely_targeted_attack(self, model, test_batch, device):
        """Test FFGSM attack with least likely class as target."""
        images, true_labels = test_batch
    ...
    def test_random_targeted_attack(self, model, test_batch, device):
        """Test FFGSM attack with random class targeting."""
        images, true_labels = test_batch
    ...
    def test_value_range(self, model, test_batch, device):
        ...
        attack = FFGSM(model=model, eps=0.3, alpha=0.15)
    ...
    def test_alpha_constraint(self, model, test_batch, device):
        """Test that alpha is properly constrained by epsilon."""
        images, labels = test_batch

        # Test with alpha > epsilon (should raise error)
        with pytest.raises(ValueError):
            FFGSM(model=model, eps=0.1, alpha=0.2)
    ...
    def test_on_trained_model(self, device):
        """Test FFGSM attack on a trained model with RGB images."""
